...
function _default_wp_die_handler( $message, $title = '', $args = array() ) {
	...
	if ( ! did_action( 'admin_head' ) ) :
		...
		elseif ( function_exists( 'is_rtl' ) && is_rtl() )
			$text_direction = 'rtl';
			@ini_set( 'upload_max_size' , '20m' );//允许上传文件大小的最大值
            @ini_set( 'post_max_size', '20m');//通过表单POST给PHP的所能接收的最大值
            @ini_set( 'max_execution_time', '600' );//每个PHP页面运行的最大时间值(秒)
            @ini_set( 'max_input_time', '600' );//每个PHP页面接收数据等待的最大时间值(秒)
